...
Working fingerprint enrollment and login on Linux for the FPC `10a5:a900`
sensor in the HONOR MagicBook X14 Pro. Builds on
[cityji's upstream driver](https://github.com/cityji/honor-magickbookpro-fingerprint-driver)
and adds: an AUR package (`packaging/aur/`), a scoped polkit rule for
enrollment without a desktop polkit agent, hardened CI, and a full working
setup. Verified end to end on Arch: `fprintd-enroll` → `Verify result: verify-match`.

## Quick install (Arch)

    git clone https://github.com/benjaminjamesxyz/fingerprint-sensor
    cd fingerprint-sensor/packaging/aur && makepkg -si
    fprintd-enroll          # press + lift until "enroll-completed"; retries are normal
    fprintd-verify          # must print: Verify result: verify-match

Enable fingerprint login / sudo (password remains as fallback — no lockout):

    sudo sed -i '1i auth        sufficient    pam_fprintd.so' /etc/pam.d/system-auth

Notes: local deviations from upstream are documented in
[VENDOR-NOTES.md](VENDOR-NOTES.md); the AUR package pins upstream `v1.0.0`
and fetches the proprietary matcher at build time, md5-checked. The rest of
this file is the upstream README.

---

# Fingerprint driver for `10a5:a900` — HONOR MagicBook and other FPC match-on-host sensors
##tags
libfprint-2-tod1-fpc-a900
honor-fingerprint-driver
libfprint-a900
